...
@Story_GPBM-3554 @Sprint16_iOS
@Story_GPBM-3604 @Sprint16_aOS

Feature: Holdings by Region at Portfolio level

  As a client I want to see the individual holdings that make up my exposure to each Region within a portfolio
  So that I know which holdings to review in order to change my Region exposure as desired

  @SIT
  Scenario: holding by Portfolio allocation region
    Given I login as type "portfolio_region_holdings_has_positive"
    Then I should on the allocation holding list

#  Scenario 1: Access holdings per Region via Allocation Graphic
  @S3554-AC1-AT1
  @DependsOn('holding_by_Portfolio_allocation_region')
  Scenario: Check the holding UI by Portfolio allocation region
...
#      | Pips indicating the number of different regions invested in (informs user that they can swipe across regions) |               |
  ...
  @SIT
  @DependsOn('holding_by_Portfolio_allocation_region')
  Scenario: Check whether the Portfolio allocation region holdings are ordered from largest to smallest based on market value in Portfolio reference currency
    Given I am on the allocation holding list
    And I should see the holdings are ordered from largest to smallest based on amount value
  ...
  @SIT
  @DependsOn('holding_by_Portfolio_allocation_region')
  Scenario: Check format of Portfolio base currency and Portfolio Total assets value for the Region in Portfolio allocation Region holding wrapper
    Given I am on the allocation holding list
  ...
  @S3554-AC1-AT6
  Scenario: Check the graphic view in Portfolio Allocation Region holding wrapper When the Region value is positive
    Given  I login as type "portfolio_region_holdings_totalValue_is_positive"
    Then the region weight is positive in the pie-char
#    Then I should see a graphic pie-char in Portfolio allocation class holding wrapper,and there are one color in the pie-char
#    And the color in graphic is same to the color that I see in the Portfolio allocation Region graphic pie-char within the current Region
#    And the position in graphic is same to the position that I see in the Portfolio allocation Region graphic pie-char within the current Region
#   Remark:can not check color

  @S3554-AC1-AT7
  Scenario: Check the graphic view in Portfolio Allocation Region holding wrapper When the Region value is negative
    Given I login as type "portfolio_region_holdings_totalValue_is_negative"
...
#    Then I should see a graphic pie-char in Portfolio Allocation Region holding wrapper
#  and there are no any color in the pie-char
  ...
  @S3554-AC1-AT8
  Scenario: Check the graphic view in Portfolio Allocation Region holding wrapper Wen the Region value is 0.00
    Given I login as type "portfolio_region_holdings_totalValue_is_zero"
#    Then I should see a graphic pie-char in Portfolio Allocation Region holding wrapper
#  and there are no any color in the pie-char
    And I tap on the Allocation toggle
    And I swipe up from the bottom of the allocation graphic page
...
#  It's been tested on @S3554-AC1-AT5
#  @S3554-AC1-AT9
#  Scenario: Check the 2dp format of region weight in Portfolio allocation region holding wrapper
#    Then I should the 2dp format of region weight
  ...
  Scenario: Check whether region name/region value/graphic pie-char for the region will be changed when I slide the pips left or right on region holdings wrapper
    Given I login as type "portfolio_region_holdings_slide_left_wrapper_change"
...
#   Remark:can not check graphic pie-char change
  ...
  Scenario: Check whether holding list will be changed when I slide the pips left or right on region holdings wrapper
    Given  I login as type "portfolio_region_holdinglist_slide_left_change"
...
#  Scenario 2: Access holdings per Region via Allocation List
  @S3554-AC2-AT1
  Scenario: Check whether I can view holding list when I tap on the tile of a particular Region in Portfolio level
    Given I login as type "portfolio_region_holdings_totalValue_is_negative"
    And I tap on the Allocation toggle
    And I swipe up from the bottom of the allocation graphic page
    When I tap on the name of a particular Asset within the allocation list
