...
@Story_GPBM-3551 @Sprint16_iOS
@Story_GPBM-3601 @Sprint16_aOS

Feature: Holdings by Currency at Portfolio level

  As a client I want to see the individual holdings that make up my exposure to each Currency within a portfolio
  So that I know which holdings to review in order to change my Currency exposure as desired

  @SIT
  Scenario: holding by Portfolio allocation currency
    Given I login as type "portfolio_currency_holdings_have_positive"
    Then I should on the allocation holding list

  #  Scenario 1: Access holdings per Currency via Allocation Graphic
  @S3551-AC1-AT1
  @DependsOn('holding_by_Portfolio_allocation_currency')
  Scenario: Check the holding UI by Portfolio allocation currency
...
#      | Pips indicating the number of different currencies invested in (informs user that they can swipe across currencies) |               |
  ...
  @SIT
  @DependsOn('holding_by_Portfolio_allocation_currency')
  Scenario: Check whether the Portfolio allocation currency holdings are ordered from largest to smallest based on market value in Portfolio reference currency
    Given I am on the allocation holding list
  ...
  @SIT
  @DependsOn('holding_by_Portfolio_allocation_currency')
  Scenario: Check whether the Portfolio currency amount equals the sum of all holdings list value
    Given I am on the allocation holding list
  ...
  @SIT
  @DependsOn('holding_by_Portfolio_allocation_currency')
  Scenario: Check format of Portfolio base currency and Portfolio Total assets value for the Region in Portfolio allocation Currency holding wrapper
    Given I am on the allocation holding list
  ...
  @S3551-AC1-AT6
  Scenario: Check the graphic view in Portfolio Allocation Currency holding wrapper When the Currency value is positive
    Given I login as type "portfolio_currency_holdings_totalValue_is_positive"
...
#    And the color in graphic is same to the color that I see in the Portfolio allocation Currency graphic pie-char within the current Currency
#    And the position in graphic is same to the position that I see in the Portfolio allocation Currency graphic pie-char within the current Currency
# Remark:can not check color

  @S3551-AC1-AT7
  Scenario: Check the graphic view in Portfolio Allocation Currency holding wrapper When the Currency value is negative
    Given I login as type "portfolio_currency_holdings_totalValue_is_negative"
...
#    Then I should see a graphic pie-char in Portfolio Allocation Currency holding wrapper
#  and there are no any color in the pie-char
    And the currency weight is negative in the pie-char
    # Remark:can not check color

  @S3551-AC1-AT8
  Scenario: Check the graphic view in Portfolio Allocation Currency holding wrapper Wen the Currency value is 0.00
...
#   It's been tested on @S3551-AC1-AT5
#  @S3551-AC1-AT9
...
#    Then I should the 2dp format of currency weight
  ...
  Scenario: Check whether currency name/currency amount/graphic pie-char for the currency will be changed when I slide the pips left or right on currency holdings wrapper
    Given I login as type "portfolio_currency_holdings_slide_left_wrapper_change"
...
#  Remark:can not check graphic color changed
  ...
  Scenario: Check whether holding list will be changed when I slide the pips left or right on currency holdings wrapper
    Given I login as type "portfolio_currency_holdinglist_slide_left_change"
...
#  Scenario 2: Access holdings per Currency via Allocation List
  @S3551-AC2-AT1
  Scenario: Check whether I can view holding list when I tap on the tile of a particular Currency in Portfolio level
    Given I login as type "portfolio_currency_holdings_totalValue_is_negative"
    And I tap on the Allocation toggle
    And I swipe up from the bottom of the allocation graphic page
    When I tap on the name of a particular Asset within the allocation list
